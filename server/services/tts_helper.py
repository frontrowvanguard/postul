"""
Supertonic TTS Helper Module
Based on https://github.com/supertone-inc/supertonic/blob/main/py/helper.py
"""
...
class UnicodeProcessor:
    """Processes Unicode text for TTS input."""
    ...
    def __call__(self, text_list: list[str]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Handle both dict and list formats for indexer
        is_dict = isinstance(self.indexer, dict)
        unk_token = self.indexer.get("<unk>", 0) if is_dict else 0
        ...
        for i, text in enumerate(text_list):
            ...
            for val in unicode_vals:
                if is_dict:
                    # Dictionary format: key is unicode value, value is token ID
                    idx = self.indexer.get(val, unk_token)
                else:
                    # List format: index is unicode value, value at index is token ID
                    # Handle out-of-bounds and -1 (unknown) values
                    if val < len(self.indexer):
                        idx = self.indexer[val]
                        if idx == -1:  # -1 means unknown
                            idx = unk_token
                    else:
                        idx = unk_token
                indices.append(idx)
            
            text_ids[i, : len(unicode_vals)] = np.array(indices, dtype=np.int64)
...
class TextToSpeech:
    """Main TTS class orchestrating all ONNX models."""
